...
class CarResourceIT {
  ...
  public static Car createEntity(EntityManager em) {
    Car car = new Car().model(DEFAULT_MODEL);
    return car;
  }
  ...
  public static Car createUpdatedEntity(EntityManager em) {
    Car car = new Car().model(UPDATED_MODEL);
    return car;
  }
  ...
  void putExistingCar() throws Exception {
    ...
    updatedCar.model(UPDATED_MODEL);
  ...
  void putNonExistingCar() throws Exception {
    ...
    restCarMockMvc
      .perform(put(ENTITY_API_URL_ID, car.getId()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(car)))
      .andExpect(status().isBadRequest());
  ...
  void patchNonExistingCar() throws Exception {
    ...
    restCarMockMvc
      .perform(
        patch(ENTITY_API_URL_ID, car.getId()).contentType("application/merge-patch+json").content(TestUtil.convertObjectToJsonBytes(car))
      )
      .andExpect(status().isBadRequest());
