...
import com.mrhopeyone.repository.ContentRepository;
...
/**
 * REST controller for managing {@link com.mrhopeyone.domain.Content}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ContentResource {
  ...
  public ResponseEntity<Content> partialUpdateContent(
    ...
    Optional<Content> result = contentRepository
      ...
      .map(existingContent -> {
        ...
        if (content.getDataContentType() != null) {
          existingContent.setDataContentType(content.getDataContentType());
        }
    ...
    return ResponseUtil.wrapOrNotFound(
      result,
      HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, content.getId().toString())
    );
  }
